...
{"release":"#{@cm.release}","db":"#{@cm.db}","taxon_level":"#{@cm.taxon_level}","protein_level":"#{@cm.protein_level}","taxon_filter":"#{@cm.taxon_filter}","protein_filter":"#{@cm.protein_filter}","taxons":[
    {"ncbi_taxon_id":"#{@cmt1.ncbi_taxon_id}","domain":"#{@cmt1.domain}","kingdom":"#{@cmt1.kingdom}","phylum":"#{@cmt1.phylum}","taxclass":"#{@cmt1.taxclass}","taxorder":"#{@cmt1.taxorder}","taxfamily":"#{@cmt1.taxfamily}","genus":"#{@cmt1.genus}","species":"#{@cmt1.species}","strain":"#{@cmt1.strain}","no_genomes":#{@cmt1.no_genomes},
      "proteins":[
    ...
    ,
    {"ncbi_taxon_id":"#{@cmt0.ncbi_taxon_id}","domain":"#{@cmt0.domain}","kingdom":"#{@cmt0.kingdom}","phylum":"#{@cmt0.phylum}","taxclass":"#{@cmt0.taxclass}","taxorder":"#{@cmt0.taxorder}","taxfamily":"#{@cmt0.taxfamily}","genus":"#{@cmt0.genus}","species":"#{@cmt0.species}","strain":"#{@cmt0.strain}","no_genomes":#{@cmt0.no_genomes},
      "proteins":[
	...
	JSON
	hash = JSON.parse(@cm.to_json)
	hash['release'].should == @cm.release
      end
    end
  end
end
